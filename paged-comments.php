...
class PagePrinter
{
  ...
  */
  ...
  function get_prev_link($text='&laquo;', $title='Previous Page')
  {
    ...
    return '<li><a class="previous-comment-page" href="'.$this->get_url($this->pager->get_current_page() - 1).'" title="'.$title.'"><i class="icon-chevron-right"></i></a></li>';
  }

  function get_next_link($text='&raquo;', $title='Next Page')
  {
    if ($this->pager->is_last_page()) return '';
    return '<li><a class="next-comment-page" href="'.$this->get_url($this->pager->get_current_page() + 1).'" title="'.$title.'"><i class="icon-chevron-left"></i></a></li>';
  }
  ...
  * @return string HTML
  */
  function get_links($separator=' ', $pre_cur_page='<li class="active"><span>', $post_cur_page='</span></li>')
  {
    ...
    if( $asc ) {
      if( $page_numbers[0] != 1 ) {
        $num_links[] = '<li><a href="' . $this->get_url(1) . '">1</a></li><li class="disabled"><span>&hellip;</span></li>';
      }
    } else {
      if( $page_numbers[0] != $this->pager->num_pages() ) {
        $num_links[] = '<li><a href="'.$this->get_url($this->pager->num_pages()). '">' .$this->pager->num_pages(). '</a></li><li class="disabled"><span>&hellip;</span></li>'; 
      }
    }
    foreach ( $page_numbers as $i) { 
      if ($i == $cur_page) { 
        ...
        $num_links[] = '<li><a href="'.$this->get_url($i)."\">$i</a></li>"; 
      } 
    } 
    if( $asc ) {
      if( $page_numbers[count($page_numbers)-1] != $this->pager->num_pages() ) {
        $num_links[] = '<li class="disabled"><span>&hellip;</span></li><li><a href="'.$this->get_url($this->pager->num_pages())."\">".$this->pager->num_pages()."</a></li>"; 
      }
    } else {
      if( $page_numbers[count($page_numbers)-1] != 1 ) {
        $num_links[] = '<li class="disabled"><span>&hellip;</span></li><li><a href="'.$this->get_url(1)."\">1</a></li>"; 
      }
    }
